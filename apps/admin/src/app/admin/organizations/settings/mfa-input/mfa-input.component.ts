...
import { AccountTypeInfo, AccountTypes } from '@coachcare/backend/services';
...
export class MFAInputComponent implements BindForm, OnDestroy, OnInit {
  ...
  private createForm() {
    ...
    this.form.controls['mfaEnabled'].valueChanges.pipe(untilDestroyed(this)).subscribe(enabled => {
      if (!enabled) {
      ...
      Object.keys(controls[sectionId]).forEach(accTypeId => {
        const existingSection = mfaPref
          ? mfaPref.sections.find(
              section => section.accountType.id === accTypeId && section.section.id === sectionId
            )
          : undefined;
        if (existingSection) {
          val.push({
          ...
          val.push({
            accountType: this.accountTypes.find(accType => accType.id === accTypeId) || null,
  ...
  private resetRawControls() {
    ...
    if (this.mfaPref) {
      this.mfaPref.sections.forEach(section => {
        const control = this.form.get(`raw.${section.section.id}.${section.accountType.id}`);
        if (control) {
  ...
  private resolveAccountTypes() {
    this.accountTypes = Object.keys(AccountTypes)
      ...
      .filter(accountType => this.ignoredAccountTypeIds.indexOf(accountType.id) === -1);
  }
  ...
  public async reload() {
    try {
      ...
      this.form.controls['mfaEnabled'].setValue(this.mfaPref ? this.mfaPref.isActive : null);
