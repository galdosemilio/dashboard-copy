...
export class ReportsListComponent implements OnInit {
  ...
  private startDate: string = moment()
    .subtract(30, 'days')
    .format('YYYY-MM-DD');
  ...
  public async downloadReport(report: 'inactiveClinics'): Promise<void> {
    try {
      this.isLoading = true;
      const response = await this.reports.fetchOrganizationActivity(this.orgActivityRequest);
  ...
  private generateCSV(orgs: any[]): string {
    ...
    orgs.forEach((entry: OrganizationActivityAggregate) => {
      ...
      csv += `"${this.generatePercent(entry.providers.active, entry.providers.total) ||
        '0'}"${separator}`;
      csv += `"${entry.clients.total || '0'}"${separator}`;
      csv += `"${entry.clients.active || '0'}"${separator}`;
      csv += `"${this.generatePercent(entry.clients.active, entry.clients.total) || '0'}"\r\n`;
    });
