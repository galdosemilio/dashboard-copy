...
export class ClinicPlanSelectorComponent implements OnDestroy, OnInit {
  @Output()
  select: EventEmitter<PlanSelectorSelectionEvent> = new EventEmitter<PlanSelectorSelectionEvent>();
  ...
  public ngOnInit(): void {
    ...
    this.route.queryParamMap.pipe(untilDestroyed(this)).subscribe((map: ParamMap) => {
      const plan = map.get('plan') || '';

      // @ts-ignore - doesn't like the way i'm comparing the map.get('billingTerm)
      // I don't like it either -- Zcyon
      const billingTermSelection: 'monthly' | 'annually' = ['monthly', 'annually'].find(
        e => e === map.get('billingTerm')
      )
        ? map.get('billingTerm')
        : 'annually';
  ...
  private attemptSelectBillingPeriod(billingPeriodName: 'monthly' | 'annually'): void {
    const foundPlan = this.clinicPlans.find(plan => plan.id === this.form.value.plan);
  ...
  private calculateClinicPlanOptions(): void {
    this.clinicPlans =
      resolveConfig('REGISTER.CLINIC_PLANS', this.context.organizationId, true) || [];
  ...
  private createForm(): void {
    ...
    this.form.controls.plan.valueChanges.pipe(untilDestroyed(this)).subscribe(planId => {
      const foundPlan = this.clinicPlans.find(clinicPlan => clinicPlan.id === planId);
      ...
      this.form.controls.billingPeriod.setValue(this.planBillingPeriods[0].value);
    });
    ...
    this.form.valueChanges
      ...
      .subscribe(controls => {
        const foundPlan = this.clinicPlans.find(clinicPlan => clinicPlan.id === controls.plan);
