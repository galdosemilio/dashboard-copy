...
import { OrganizationDetailed, PackageEnrollmentSegment } from '@app/shared/selvera-api';
...
export class PhasesComponent implements OnDestroy, OnInit {
  ...
  private createSource(): void {
    ...
    this.source.addRequired(this.context.organization$.pipe(debounceTime(200)), () => ({
      organization: this.context.organizationId
    }));
    this.source.addRequired(this.context.account$.pipe(debounceTime(200)), () => ({
      account: this.context.accountId
    }));

    this.source.addOptional(this.enrollmentsScroll$.pipe(debounceTime(200)), () => ({
  ...
  private onPhaseAdded(phase: LabelsDataSegment): void {
    const phaseHierarchyPath = (phase.package.organization as OrganizationDetailed)
      .hierarchyPath;

    if (
      intersectionBy(phaseHierarchyPath, this.context.organization.hierarchyPath, Number)
  ...
  private subscribeToScroll(): void {
    this.onScroll$.pipe(debounceTime(100), untilDestroyed(this)).subscribe(this.onScroll);
  }
