...
export class CallSettingsComponent implements OnDestroy, OnInit {
  ...
  deviceInfo: any;
  form: FormGroup;
  filledMicBlocks: number = 0;
  micBlocksAmount: number = 10;
  micBlocks: any[] = [];
  previewState: any = {
    video: {
      deviceId: '',
  ...
  constructor(
    ...
    this.store.pipe(select(callSelector)).subscribe((callState) => {
      ...
      this.canPlaySound =
        this.callState.audioOutputDevices && this.callState.audioOutputDevices.length > 0;
    });
  }
  ...
  ngOnInit() {
    ...
    this.createForm();
    this.store.pipe(select(callSelector), untilDestroyed(this)).subscribe((callState) => {
      const value = this.form.value;
      this.form.patchValue({
        selectedAudioInputDevice:
          value.selectedAudioInputDevice || callState.selectedAudioInputDevice,
        selectedVideoInputDevice:
          value.selectedVideoInputDevice || callState.selectedVideoInputDevice,
        selectedAudioOutputDevice:
          value.selectedAudioOutputDevice || callState.selectedAudioOutputDevice
      });
    });
  ...
  createForm() {
    ...
    this.form.valueChanges
      ...
      .subscribe((controls) => {
        ...
        this.previewState.audioOutput.deviceId = controls.selectedAudioOutputDevice;
      });
  ...
  onApply() {
    if (
      this.callState.selectedAudioInputDevice !== this.form.value.selectedAudioInputDevice
    ) {
      this.store.dispatch(
    ...
    if (
      this.callState.selectedVideoInputDevice !== this.form.value.selectedVideoInputDevice
    ) {
      this.store.dispatch(
    ...
    if (
      ...
      this.store.dispatch(
        new ApplySelectedAudioOutputDevice(this.form.value.selectedAudioOutputDevice)
      );
    }
  ...
  async onTestAudioOutput() {
    if (this.previewState.audioOutput.deviceId) {
      await this.testAudioSound.setSinkId(`${this.previewState.audioOutput.deviceId}`);
