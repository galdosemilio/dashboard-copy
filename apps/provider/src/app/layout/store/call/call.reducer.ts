...
export function callReducer(state = initialState, action: Action): CallState {
  switch (action.type) {
    ...
    case Actions.INITIATE_CALL:
      return {
        ...state,
        billableService: (action as Actions.InitiateCall).payload
          .billableService,
        isCallStarted: true,
    ...
    case Actions.RECEIVE_CALL:
      return {
        ...
        hasVideoStarted: false,
        source:
          (action as Actions.ReceiveCall).payload.source || Source.INBOUND,
    ...
    case Actions.CHECK_DEVICES_COMPLETE:
      ...
      return {
        ...
        hasVideoDeviceAccess: (action as Actions.CheckDevicesComplete).payload
          .video,
        hasAudioDeviceAccess: (action as Actions.CheckDevicesComplete).payload
          .audio,
    ...
    case Actions.SET_ATTEMPTING_RECONNECT:
      return {
        ...state,
        isAttemptingToReconnect: (action as Actions.SetAttemptingReconnect)
          .payload
      }
...
function updateParticipantIds(
  ...
  return {
    ...
    participants: state.participants.map((participant) => {
      if (
        response.participants.attended.filter((x) => x.id === participant.id)
          .length > 0
      ) {
        return {
          ...participant,
