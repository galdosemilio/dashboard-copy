...
import { ContextService, NotifierService, TimeTrackerService } from '@app/service';
import {
  AccountAccessData,
...
export class RPMTrackerComponent implements OnDestroy, OnInit {
  ...
  public timerDisplay: string = '00:00';
  public timeTrackingComplete: boolean = false;
  ...
  private _iterationAmount: number = 0;
  private requiredIterationSeconds: number = 1200;
  ...
  private async resolveAccountRPMStatus(account: AccountAccessData): Promise<void> {
    try {
      if (!account) {
        return;
      }
  ...
  private resolveCurrentRPMBillingCode(
    ...
    if (!billingItem) {
      billingItem = stateSummaryItem.billing[stateSummaryItem.billing.length - 1];
  ...
  private async resolveRPMBillingStatus(account: AccountAccessData): Promise<void> {
    try {
      if (!account) {
        return;
      }
