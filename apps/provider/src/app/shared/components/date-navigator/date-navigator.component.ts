...
export class DateNavigator implements OnChanges {
  ...
  @Input() showTimeframeSelector: boolean = false;
  ...
  private processAndEmit(): void {
    ...
    this._start = this.discrete ? moment(end).startOf(this.timeframe) : this._start;
    ...
    switch (this.timeframe) {
      case 'day':
        ...
        break;
      case 'week':
        this._range = `${this._start.format('MMM DD')} - ${end.format('DD, YYYY')}`;
        break;
      case 'month':
      ...
      case 'year':
        this._range = this.discrete
          ? `${end.format('YYYY')}`
          : `${this._start.format('MMM DD, YYYY')} - ${end.format('MMM DD, YYYY')}`;
        break;
    }
