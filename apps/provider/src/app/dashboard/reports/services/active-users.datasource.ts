...
> {
  ...
  mapChart(result: Array<OrganizationActivityAggregate>): ChartData {
    ...
    const uniqueDates = result.map((v) => v.date).filter((v, i, s) => s.indexOf(v) === i);
    ...
    const totals = uniqueDates.forEach((date) => {
      ...
      data.inactiveClientsPercent.push(100 - (activeClients / totalClients) * 100);
      ...
      data.inactiveProvidersPercent.push(100 - (activeProviders / totalProviders) * 100);
