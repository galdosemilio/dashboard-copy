...
export class CallHistoryItem {
  ...
  constructor(args: any) {
    ...
    const interactionType = args.type
      ? Object.values(INTERACTION_TYPES).find((type) => type.id === args.type.id)
      : null;
    ...
    this.type = args.type
      ? {
          ...args.type,
          displayName: interactionType ? interactionType.displayName : args.source.name
        }
      : {
          ...args.source,
    ...
    this.canUpdateRpmBilling = moment().isSameOrBefore(moment(this.time.start), 'month');
    this.canBeDeleted = this.type && this.type.id === '2' && this.canUpdateRpmBilling;
  }
}
