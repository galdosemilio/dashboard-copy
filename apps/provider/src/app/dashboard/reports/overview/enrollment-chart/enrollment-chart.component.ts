...
import { EnrollmentSimpleReportResponse, TimelineUnit } from '@app/shared/selvera-api';
...
export class EnrollmentChartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
      ...
      try {
        ...
        this.simpleReport = await this.database.fetchSimpleEnrollmentReport({
          ...
          range: {
            ...
            end: this.data ? moment(this.data.endDate).add(1, 'day').toISOString() : null
          },
          enrollmentLimit: 'all',
          limit: 'all'
        });
    ...
    this.context.organization$.pipe(untilDestroyed(this)).subscribe((org) => {
      if (this.data && this.source.isLoaded && !this.source.isLoading && org.id) {
  ...
  private preprocessSimpleReportElements(array: any[]): any[] {
    ...
    array.forEach((data) => {
      ...
      const existsWithOrganization =
        ...
        items.find(
          (it) =>
            ...
            it.enrollments.find(
              (enrollment) => enrollment.package.id === data.enrollment.package.id
            )
        );
      ...
      if (!existsWithOrganization) {
        items.push({
          ...data,
          externalIdentifiers: data.externalIdentifier ? [data.externalIdentifier] : [],
      ...
      if (existing) {
        ...
        existingElements.forEach((element) => {
          if (element.externalIdentifiers && element.externalIdentifiers.length) {
        ...
        existingElements.forEach(
          (element) => (element.externalIdentifiers = groupedExternalIdentifiers)
        );
      }
    });
      ...
      if (item.enrollments && item.enrollments.length) {
        ...
        if (individualIdentifiers && individualIdentifiers.length) {
          ...
          individualIdentifiers.forEach((identifier, index) => {
            const displayName = this.identifierNames[identifier.name] || undefined;
            csv += displayName
        ...
        csv += item.enrollments.reduce((cell, enrollment, index) => {
          cell +=
            enrollment.range && enrollment.range.start ? `${enrollment.range.start}` : '';
          cell += `${index < item.enrollments.length - 1 ? '\n' : ''}`;
          return cell;
        }, '');
        csv += `"`;
        ...
        csv += item.enrollments.reduce((cell, enrollment, index) => {
          cell +=
            enrollment.range && enrollment.range.end ? `${enrollment.range.end}` : '';
          cell += `${index < item.enrollments.length - 1 ? '\n' : ''}`;
          return cell;
        }, '');
        csv += `"`;
  ...
  private resolveIdentifierNames() {
    ...
    if (
      ...
      this.translate
        .get(
          resolvedConfig.values.identifiers.reduce((names, currentIdentifier) => {
        ...
        .subscribe((translations) => {
          resolvedConfig.values.identifiers.forEach((identifier) => {
            this.identifierNames[identifier.name] = translations[identifier.displayName]
