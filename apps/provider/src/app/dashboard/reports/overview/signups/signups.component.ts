...
import { TimelineUnit } from '@app/shared/selvera-api';
...
export class SignupsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.context.organization$.pipe(untilDestroyed(this), delay(1)).subscribe((org) => {
      if (this.data && org.id) {
        this.refresh$.next();
      }
    });
  ...
  downloadCSV() {
    ...
    csv +=
      dates.map((d) => `"${moment(d).format(format)}"`).join(this.csvSeparator) + '\r\n';
