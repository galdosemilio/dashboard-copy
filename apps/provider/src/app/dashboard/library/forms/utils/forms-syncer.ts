...
export class FormsSyncer {
      ...
      try {
        ...
        sections.forEach((s: FormSection) => {
          ...
          if (changeTicket.action === CHANGE_TICKET_ACTIONS.delete) {
            ...
            s.questions.forEach((q: FormQuestion) =>
              changeTickets.push(this.checkElementChanges(q, CHANGE_TICKET_TYPE.question))
            );
          } else {
            s.questions.forEach((q: FormQuestion) =>
              changeTickets.push(this.checkElementChanges(q, CHANGE_TICKET_TYPE.question))
        ...
        while (changeTickets.length) {
          ...
          ticket.payload =
            ticket.dependencies && ticket.dependencies.length ? response : ticket.payload;
  ...
  private checkElementChanges(
    ...
    if (
      ...
      !QUESTION_TYPE_MAP[changeTicket.payload.questionType.id].requiresValueList &&
      !QUESTION_TYPE_MAP[changeTicket.payload.questionType.id].requiresNumericRange
    ) {
