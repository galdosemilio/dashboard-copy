...
export class FormSubmissionsComponent implements OnDestroy, OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.source.addOptional(this.dates$, () => ({ createdAt: { ...this.dates } }));
    this.source.addOptional(this.organization$, () => ({
      organization: this.organization ? this.organization.id : this.context.organizationId
    }));

    this.source
      .connect()
      .pipe(untilDestroyed(this), debounceTime(100))
      .subscribe((submissions) => (this.hasSubmissions = submissions.length > 0));
  }
  ...
  public async onGenerateCSV() {
    try {
      ...
      submissions.forEach((submission: any, index) => {
        ...
        csv += `"${moment(submission.createdAt).format('h:mm a')}"${this.csvSeparator}`;
        ...
        this.form.sections.forEach((section) => {
          section.questions.forEach((question, questionIndex) => {
            ...
            csv += `"${(ans && ans.response.value) || ' '}"${
              questionIndex >= section.questions.length - 1 ? '' : this.csvSeparator
            }`;
          });
        });
