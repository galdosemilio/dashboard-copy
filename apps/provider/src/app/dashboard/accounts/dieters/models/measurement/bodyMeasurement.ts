...
import { AccountMeasurementPreferenceType, Entity } from '@app/shared/selvera-api';
...
export class BodyMeasurement {
  ...
  constructor(args: any, opts: BodyMeasurementConstructorOptions = {}) {
    ...
    this.weight = unitConversion(units, 'composition', Number(args.weight || 0), false);
    this.arm = unitConversion(units, 'circumference', Number(args.arm || 0), false);
    ...
    this.bodyFat =
      this.weight && this.bodyFatPercentage
        ...
        : 0;
    this.chest = unitConversion(units, 'circumference', Number(args.chest || 0), false);
    ...
    this.hip = unitConversion(units, 'circumference', Number(args.hip || 0), false);
    ...
    this.ldl = args.ldl || 0;
    this.leanMassPercentage = this.bodyFatPercentage ? 100 - this.bodyFatPercentage : 0;
    this.leanMass = this.weight && this.bodyFat ? this.weight - this.bodyFat : 0;
    this.neck = unitConversion(units, 'circumference', Number(args.neck || 0), false);
    ...
    this.temperature =
      this.temperature !== 0
        ...
        : 0;
    this.thigh = unitConversion(units, 'circumference', Number(args.thigh || 0), false);
    this.thorax = unitConversion(units, 'circumference', Number(args.thorax || 0), false);
    ...
    this.waist = unitConversion(units, 'circumference', Number(args.waist || 0), false);
    this.waterPercentage = Number(args.waterPercentage ? args.waterPercentage / 1000 : 0);
    this.water =
  ...
  static calculateAverage(
    ...
    const average: BodyMeasurement = new BodyMeasurement(
      { recordedAt: measurements[0].recordedAt, updatedAt: measurements[0].updatedAt },
      opts
    );
