...
export class SupplementDataSource extends CcrDataSource<any, any, SupplementCriteria> {
  supplements: Array<FetchSupplementsSegment> = [];
  columns: Array<string> = [];

  constructor(protected notify: NotifierService, protected database: SupplementDatabase) {
    super();
  }
  ...
  fetch(
    ...
    return observableThrowError('Missing Organization to fetch Supplements from.');
  }

  mapResult(result: [FetchSupplementsResponse, FetchSupplementSummaryResponse]) {
