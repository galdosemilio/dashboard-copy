...
> {
  ...
  mapResult(result: GetAllFoodConsumedResponse): FoodDayAmount[] {
    const parsedData =
      result.data && result.data.length
        ? result.data.map((element: FoodConsumedSingle) => new ConsumedFood(element))
        : [];
  ...
  private sortBy(array) {
    ...
    Object.keys(this.TYPES).forEach((key) => {
      sortByData = sortByData.concat(
        array.filter((item) => this.translateType(item.type) === this.TYPES[key])
      );
    });
    return sortByData;
  }
  ...
  private addEmptyDays(groupedFood) {
    ...
    for (
      ...
      if (
        !groupedFood.some((day) => moment(day[0].consumedDate).isSame(currentDate, 'day'))
      ) {
