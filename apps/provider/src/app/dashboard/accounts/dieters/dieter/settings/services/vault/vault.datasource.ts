...
import { ContentCopiedEvent, FileExplorerContent } from '@app/dashboard/content/models';
...
> {
  ...
  fetch(criteria: GetAllVaultContentRequest): Observable<GetAllVaultContentResponse> {
    return this.database.fetch(criteria);
  }

  async mapResult(result: GetAllVaultContentResponse): Promise<FileExplorerContent[]> {
    ...
    const perms = fromPairs(
      await Promise.all(
        ids.map(async (id) => [id, await this.context.orgHasPerm(id, 'allowClientPhi')])
      )
    );

    let fileExplorerSortOrder: number = 0;
    const opts = { organizationId: this.criteria.organization };
    const data = result.data
      .map((r: ContentSingle) => {
        const localSortOrder = r.sortOrder ? false : true;
        fileExplorerSortOrder = r.sortOrder ? r.sortOrder : ++fileExplorerSortOrder;
  ...
  createContent(args: CreateVaultContentRequest): Promise<FileExplorerContent> {
    ...
    return this.execRequest(
      this.database
        ...
        .pipe(map((content: ContentSingle) => new FileExplorerContent(content, opts)))
        .toPromise()
    );
  }

  createContentPackage(args: CreateContentPackageRequest): Promise<void> {
    return this.execRequest(this.database.createContentPackage(args).toPromise());
  }
  ...
  updateContent(args: UpdateContentRequest): Promise<FileExplorerContent> {
    ...
    return this.execRequest(
      this.database
        ...
        .pipe(map((content: FileExplorerContent) => new FileExplorerContent(content)))
        .toPromise()
    );
  }
  ...
  deleteContentPackage(args: DeleteContentPackageRequest): Promise<void> {
    return this.execRequest(this.database.deleteContentPackage(args).toPromise());
  }
  ...
  getUploadUrl(args: GetUploadUrlContentRequest): Promise<GetUploadUrlContentResponse> {
