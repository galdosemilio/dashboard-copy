...
import { ContentPackage, FileVault } from 'selvera-api';
import {
...
} from 'selvera-api/dist/lib/selvera-api/providers/content';
...
export class VaultDatabase extends CcrDatabase {
  ...
  public createContent(args: CreateVaultContentRequest): Observable<ContentSingle> {
    return from(
  ...
  public fetch(args: GetAllVaultContentRequest): Observable<GetAllVaultContentResponse> {
    ...
    return from(
      new Promise<FileExplorerContent>(async (resolve, reject) => {
        try {
          ...
          const content: ContentSingle = await this.fileVault.getSingle({ id: args.id });
          resolve({
            ...content,
