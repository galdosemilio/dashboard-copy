...
> {
  ...
  async mapResult(result: GetAllContentResponse): Promise<FileExplorerContent[]> {
  ...
  public async copyContent(args: ContentCopiedEvent): Promise<FileExplorerContent> {
    try {
      ...
      const { name, ...updateParams } = { ...copiedSingle, ...args.overrideDetails };
