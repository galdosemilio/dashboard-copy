...
export class YoutubeFormComponent implements BindForm {
  ...
  constructor(private domSanitizer: DomSanitizer, private formBuilder: FormBuilder) {
    this.createForm();
  }
  ...
  private createForm(): void {
    ...
    this.form.valueChanges.pipe(debounceTime(700)).subscribe((controls: any) => {
  ...
  private calculateEmbeddedUrl(controls: any): void {
    try {
      ...
      this.form.patchValue({ url: this.url.changingThisBreaksApplicationSecurity });
  ...
  private processAsDefault(url: string): { id: string; startTime?: number } {
    ...
    return {
      id: params[0].split('=')[1],
      startTime:
        url.indexOf('&') > -1 ? Number(params[1].split('=')[1].split('s')[0]) : undefined
    };
  }
}
