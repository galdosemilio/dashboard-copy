...
export class SequencingFormComponent implements ControlValueAccessor, OnDestroy, OnInit {
  ...
  onAddStep(initialValue: SequenceState | null = null): void {
    ...
    stepsControl.setValidators((control) => {
      if (
        !control.value ||
        control.value.find((stepObject) => !stepObject.step || !stepObject.step.valid)
      ) {
  ...
  onDelayChanged(value: string, index: number): void {
    ...
    (this.form.controls.steps as FormGroup).controls[index].patchValue(stepValue);
    this.forceDelayRecalculation();
  }
  ...
  onStepDeleted(index: number): void {
    ...
    (this.form.controls.steps as FormArray).controls[index].patchValue({
      step: {
        ...
        syncState: {
          ...(this.form.controls.steps as FormArray).controls[index].value.step.syncState,
          deleted: true
        }
      }
    });
  ...
  private createStepGroup(initialValue: SequenceState | null = null): FormGroup {
    ...
    this.form.valueChanges
      ...
      .subscribe((controls) => {
        ...
        this.propagateChange({
          ...controls,
          steps: rootState ? [{ step: rootState }, ...controls.steps] : controls.steps
        });
        this.form.patchValue(
          {
        ...
        if (steps && steps.length) {
          steps.forEach((step, index) => {
            if (!step.step || !step.step.syncState || !step.step.syncState.deleted) {
      ...
      .subscribe((controls) => {
        ...
        controls.forEach((stepObject, index) => {
          ...
          const initialTransition = this.initialValue
            ? this.initialValue.transitions.find((t) => t.to.id === stepObject.step.id)
            : undefined;
          ...
          if (transition && stepObject.step) {
            transition.syncState.edited = initialTransition
              ...
              : true;
            transition.delay = stepObject.step.serverDelay || stepObject.step.delay;
          }
        });
  ...
  private resolveEndingActions(): void {
    this.endingActions = Object.keys(EndingActions).map((key) => EndingActions[key]);
  }
        ...
        if (!remainingHours.length) {
          ...
          stepValue.step.delay =
            stepValue.step.delay && stepValue.step.delay !== 'no delay'
              ? stepValue.step.delay
              : this.availableStepDelays[this.availableStepDelays.length - 1][0].name;
          stepValue.step.syncState = {
            ...stepValue.step.syncState,
            edited: stepValue.step.syncState.edited
              ? true
    ...
    if (controls && controls.steps && controls.steps.length) {
      ...
      controls.steps.forEach((stepObject, index) => {
        ...
        const hourDiff = Math.abs(
          prevHour.diff(
            !step.delay || isAfterPrev ? currentHour : currentHour.clone().add(1, 'day'),
            'hours'
          )
        );
      ...
      if (stepControls.length) {
        stepControls.controls.forEach((control, index) => {
          ...
          const delayHour = control.value.step.delayHour
            ? SEQUENCE_HOURS.find((hour) => control.value.step.delayHour === hour.value)
                .displayName
