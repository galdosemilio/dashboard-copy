...
import { AuthService, ContextService, NotifierService } from '@coachcare/common/services';
import { AuthenticatorApp, AuthenticatorApps, MFAChannel, MFAChannels } from './models';

type MFASetupPageModes = 'auth' | 'backup_codes' | 'sms' | 'default' | 'backup_codes';
...
export class MFASetupPageComponent implements OnInit {
  ...
  async onAccept() {
    try {
      ...
      const qrDataMatch = this.qrData ? this.qrData.match(/secret\=[a-zA-Z0-9]*/) : '';
