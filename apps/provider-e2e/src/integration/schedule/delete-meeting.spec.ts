...
describe('Schedule -> delete meeting', function () {
  it('List View -> Single meeting delete', function () {
    ...
    cy.get('@modalContent').contains('Are you sure you want to delete this meeting?');
  ...
  it('Calendar View -> Single meeting delete', function () {
    ...
    cy.get('@modalContent').get('.ccr-button.mat-button').contains('Delete').click();
...
function listViewRecurringMeeting(option: Option): void {
  ...
  // Get the last available delete button
  cy.get('[data-cy="schedule-listing-meeting-delete"]').last().as('meetingDeleteButton');
  ...
  cy.get('@modalContent').get('button').contains('Delete').click({ force: true });
...
function calendarViewRecurringMeeting(option: Option): void {
  ...
  cy.get('div.meeting.meeting-selectable').contains('Test meeting 5').as('calendarBody');
  cy.get('@calendarBody').click({ force: true });
