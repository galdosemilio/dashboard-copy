...
describe('Lefthand menu (standard)', function () {
  ...
  it('By default, all proper links show', function () {
    ...
    cy.get('app-menu', {
      ...
      .not('.hidden')
      .as('menuLinks')
    cy.get('@menuLinks').should('have.length', 27)
    ...
    cy.get('@menuLinks').eq(19).should('contain', 'Device')
    cy.get('@menuLinks').eq(20).should('contain', 'Sequences')
    cy.get('@menuLinks').eq(21).should('contain', 'Store')
    cy.get('@menuLinks').eq(22).should('contain', 'Store')
    cy.get('@menuLinks').eq(23).should('contain', 'Resources')
    cy.get('@menuLinks').eq(24).should('contain', 'Updates')
    cy.get('@menuLinks').eq(25).should('contain', 'Contact Support')
    cy.get('@menuLinks').eq(26).should('contain', 'FAQ & Support Guides')
  ...
  it('Translates left menu on language change', function () {
    ...
    cy.get('@menuLinks').eq(23).should('contain', 'Recursos')
    cy.get('@menuLinks').eq(24).should('contain', 'Novedades')
    cy.get('@menuLinks').eq(25).should('contain', 'Soporte de contacto')
    cy.get('@menuLinks').eq(26).should('contain', 'FAQ y Guías')
  })
})
