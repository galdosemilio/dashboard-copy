...
describe('Patient profile -> clinics', function () {
  ...
  it('Clinics associations exist for patient, and show correctly', function () {
    cy.visit(`/accounts/patients/${Cypress.env('clientId')}/settings;s=associations`);

    cy.get('app-dieter-settings').find('li').contains('Clinics').should('have.length', 1);

    cy.get('ccr-account-associations').find('mat-row').as('clinicRows');

    cy.get('@clinicRows').eq('0').should('contain', 'CoachCare');

    cy.get('@clinicRows').eq('1').should('contain', 'Center for Medical Weight Loss');
  });
  ...
  it('Clinics associations can be added for patient', function () {
    ...
    cy.get('@clinicOptions').eq(2).should('contain', 'Center for Medical Weight Loss');
  ...
  it('Deletable clinics associations limited to only "admin" permissioned clinic', function () {
    ...
    cy.get('@clinicRows').eq('0').find('mat-icon').should('have.class', 'disabled');

    cy.get('@clinicRows').eq('1').find('mat-icon').should('have.class', 'disabled');
  });
  ...
  it('Cannot add association if no "admin" permissioned clinics', function () {
    ...
    cy.get('ccr-account-associations').find('button').should('have.attr', 'disabled');
  });
});
