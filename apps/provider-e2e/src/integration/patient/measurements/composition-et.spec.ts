...
describe('Patient profile -> measurement -> composition (ET)', function () {
  ...
  it('Table shows weight data in ET (New York)', function () {
    ...
    cy.get('app-dieter-measurements-table').find('tbody tr').as('measurementRows');
  ...
  it('Add composition measurement in ET (New York)', function () {
    ...
    cy.get('@categoryButtons').eq(1).should('contain', 'Composition').click('left');

    cy.get('add-rightpanel-measurements').find('[formcontrolname="weight"]').type('185');

    cy.get('add-rightpanel-measurements').find('a').contains('Add Measurement').click();
