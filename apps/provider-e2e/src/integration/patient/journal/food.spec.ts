...
describe('Patient profile -> journal -> food', function () {
  it('Summary circles show with correct labels', function () {
    ...
    cy.get('@summaryBoxes').eq(3).should('contain', 'FAT').should('contain', 'Sat. Fat');
  ...
  it('Values show correct in Eastern Time (New York)', function () {
    ...
    cy.get('@summaryBoxes')
      ...
      .should('contain', '3.0g')
      .should('contain', '0g');
    cy.get('@summaryBoxes').eq(3).should('contain', '3.0g').should('contain', '0g');

    cy.get('app-dieter-journal-food-table').find('mat-row.level0').as('tableRows');
