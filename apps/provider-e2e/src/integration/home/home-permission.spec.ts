...
describe('Validate permissions on homepage', function() {
  ...
  it('"View All" and "Client PHI" permission for associated organization', function() {
    ...
    cy.get('[data-cy="patient-listing-actions-button-open"]').should('have.length', 2);
    cy.get('[data-cy="patient-listing-actions-button-edit"]').should('have.length', 2);
    cy.get('[data-cy="patient-listing-actions-button-delete"]').should('have.length', 0);
  });
  ...
  it('Full permissions for associated organization', function() {
    ...
    cy.get('[data-cy="patient-listing-actions-button-open"]').should('have.length', 2);
    cy.get('[data-cy="patient-listing-actions-button-edit"]').should('have.length', 2);
    cy.get('[data-cy="patient-listing-actions-button-delete"]').should('have.length', 2);
  });
});
