...
describe('Lefthand menu (CMWL)', function () {
  ...
  it('Test custom case: CMWL, all proper links show', function () {
    ...
    cy.get('@menuLinks').eq(10).should('contain', 'Alerts')
    cy.get('@menuLinks').eq(11).should('contain', 'Notifications')
    cy.get('@menuLinks').eq(12).should('contain', 'Settings')
    cy.get('@menuLinks').eq(13).should('contain', 'Reports')
    cy.get('@menuLinks').eq(14).should('contain', 'RPM')
