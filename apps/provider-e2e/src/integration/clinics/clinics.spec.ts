...
describe('Dashboard -> Clinics', function () {
  it('Creates a new clinic', function () {
    ...
    cy.wait('@organizationCreateRequest').should((xhr) => {
      ...
      expect(xhr.request.body.contact.firstName).to.equal('cypress_contact_firstname');
      expect(xhr.request.body.contact.lastName).to.equal('cypress_contact_lastname');
      ...
      expect(xhr.request.body.address.postalCode).to.equal('cypress_address_postal_code');
