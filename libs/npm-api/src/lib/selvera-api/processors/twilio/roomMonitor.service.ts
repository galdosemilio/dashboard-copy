...
export class TwilioRoomMonitor {
        ...
        this.subscriptions = [
            this.connectionUpdate$
                ...
                .subscribe(async connectionStats => {
                    try {
                        await this.checkLocalConnectionForLogging(
                            connectionStats
                        );
                    } catch (error) {
                        // we fail silently
    ...
    public monitorRoomHandler(roomStats: StatsReport[]): void {
        ...
        Object.keys(BandwidthEntryType).forEach(key => {
            const bytesType = key.includes('Remote')
                ? 'bytesReceived'
                : 'bytesSent';
            ...
            if ((this.stats[0] as any)[type].length) {
                ...
                newStatsEntry.deltaBytesReceived =
                    newStatsEntry.bytesReceived -
                    this.previousRawStatsEntry.bytesReceived;
                newStatsEntry.deltaBytesSent =
                    newStatsEntry.bytesSent -
                    this.previousRawStatsEntry.bytesSent;
            }
        });
        ...
        const connectionStats = new ConnectionStats(newStatsEntry, {
            ...
            connectionStatsHistory:
                historyIndex > -1
                    ? this.connectionStatsHistory[historyIndex]
                    : [],
        ...
        switch (mess.type) {
            ...
            case TwilioDataMessageType.MOB_CAM_OFF:
                ...
                remoteConn.hasMobileVideoEnabled =
                    mess.type === TwilioDataMessageType.MOB_CAM_ON
                        ? true
                        : false;
        ...
        try {
            ...
            if (
                ...
                (this.previousLoggedConnStats &&
                    this.previousLoggedConnStats.status ===
                        localConnStats.status)
            ) {
                return;
            }
            ...
            await this.logging.add({
                app: this.appType,
                logLevel: 'info',
                message: `${localConnStats.id} connection changed to ${
                    localConnStats.status
                }`,
    ...
    private upsertConnectionStatsHistory(
        connectionStats: ConnectionStats
    ): void {
        ...
        if (index > -1) {
            ...
            if (
                this.connectionStatsHistory[index].length > this.historyAmount
            ) {
    ...
    private upsertConnectionStats(newConnectionStats: ConnectionStats): void {
        ...
        if (hitIndex > -1) {
            if (
                this.allConnectionStats[hitIndex].status ===
                ConnectionStatus.IDLE
            ) {
