...
export class DieterDashboardSummary {
    ...
    private calcBMRStat(activityLevel?: number): void {
        ...
        this.BMR =
            data.gender === 'female'
                ? Math.round(
                      (weightKg * 9.25 + 447.6 + heightCm * 3.1 - age * 4.33) *
                          factor
                  )
                : Math.round(
                      (weightKg * 13.4 + 88.4 + heightCm * 4.8 - age * 5.68) *
                          factor
                  );
    ...
    private getMetricData(
        ...
        return Promise.all([
            ...
            .then(([summary, goal, dieter]) => {
                ...
                const weightGoal =
                    goal && goal.goal && goal.goal.weight
                        ? goal.goal.weight
                        : null;
        ...
        if (data.weight) {
            ...
            if (data.bodyFat) {
                ...
                this.starting.bodyFat =
                    (sweight * bodyFat.first.value) / 100000;
            ...
            if (data.leanMass) {
                ...
                this.starting.leanMass =
                    (sweight * leanMass.first.value) / 100000;
                this.current.leanMass =
                    (cweight * leanMass.last.value) / 100000;
            }
            if (data.hydration) {
