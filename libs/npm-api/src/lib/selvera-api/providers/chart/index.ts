...
import { ActivityMeasurementRequest, BodyMeasurementRequest, CalorieRequest, SleepRequest } from './requests';
import {
    ActivityResponse,
    BmiResponse,
    BodyFatResponse,
...
class Chart {
     ...
     */
    public async activity(fetchActivityRequest: ActivityMeasurementRequest): Promise<ActivityResponse> {
        ...
        const goalRequest: FetchGoalRequest = {
            account: fetchActivityRequest.account !== undefined ? fetchActivityRequest.account : undefined
        };
        ...
        return Promise.all(request).then(response => {
            ...
            return {
                data: activity.data,
                summary: {
                    current: activity.data.length ? activity.data.slice().reverse()[0].stepTotal : 0,
    ...
    public async sleep(fetchSleepRequest: SleepRequest): Promise<SleepResponse> {
        ...
        const goalRequest: FetchGoalRequest = {
            account: fetchSleepRequest.account !== undefined ? fetchSleepRequest.account : undefined
        };
        ...
        return Promise.all(request).then(response => {
            ...
            const current = sleep.data.length
                ? moment.duration(sleep.data.slice().reverse()[0].sleepMinutes, 'second').asHours()
                : 0;
            const average = sleep.data.length
                ? moment.duration(sleep.data.slice().reverse()[0].averageMinutes, 'second').asHours()
                : 0;
     ...
     */
    public async calorie(fetchCalorieRequest: CalorieRequest): Promise<CalorieResponse> {
        ...
        return this.apiService
            ...
            .then(response => {
                ...
                const average = meals.length
                    ? Math.round(meals.reduce((sum: any, value: any) => sum + value.calories, 0) / meals.length)
                    : 0;
     ...
     */
    public async leanMass(fetchLeanMassRequest: BodyMeasurementRequest): Promise<LeanMassResponse> {
        ...
        return this.apiService
            ...
            .then(response => {
                const data = response.data.map((item: any) => {
                    ...
                    const fatMassWeight = Math.round((item.weight * item.bodyFat) / 100000);
                    if (item.weight > 0 && fatMassWeight > 0) {
                        percentage = Math.round(((item.weight - fatMassWeight) / item.weight) * 1000) / 10;
     ...
     * @returns BmiResponse
     */
    public async bmi(fetchBmiRequest: BodyMeasurementRequest): Promise<BmiResponse> {
     ...
     */
    public async bodyFat(fetchBodyFatRequest: BodyMeasurementRequest): Promise<BodyFatResponse> {
     ...
     */
    public async weight(fetchWeightRequest: BodyMeasurementRequest): Promise<WeightResponse> {
                    ...
                    return {
                        ...
                        leanMass:
                            item.weight > 0 && item.bodyFat > 0
                                ? item.weight - Math.round((item.weight * item.bodyFat) / 100000)
                                : 0
                    };
                });
     ...
     */
    public async circumference(fetchCircumferenceRequest: BodyMeasurementRequest): Promise<CircumferenceResponse> {
        fetchCircumferenceRequest.data = ['waist', 'arm', 'hip', 'chest', 'thigh', 'neck', 'thorax'];
