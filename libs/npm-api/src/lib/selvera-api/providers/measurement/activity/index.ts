...
class MeasurementActivity {
  ...
  public fetchActivity(
    ...
    return this.apiService
      ...
      .then((res) => {
        const response = res.map((segment: FetchUnfilteredActivityResponse) => {
          return {
            id: segment.id,
  ...
  public fetchHistory(
    ...
    return Promise.all(request).then((response) => {
      ...
      measurement.history.map((i: ActivityHistoryResponseSegment) => {
        i.date = moment.utc(i.date).tz(user.timezone).format()
        i.recordedAt = moment.utc(i.recordedAt).tz(user.timezone).format()
        return i
      })
  ...
  public fetchSummary(
    ...
    return this.apiService
      ...
      .then((res) => {
        res.data = res.data.map((segment: SummaryActivityResponseSegment) => {
  ...
  public getSummary(
    ...
    return this.apiService
      ...
      .then((res) => {
        res.data = res.data.map((segment: MeasurementActivitySegment) => {
          segment.date = moment.utc(segment.date).format('YYYY-MM-DD')
          return segment
        })
