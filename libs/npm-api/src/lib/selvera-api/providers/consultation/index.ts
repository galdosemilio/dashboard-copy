...
import { ConsultationCreateRequest, ConsultationListingRequest } from './requests';
...
class Consultation {
    ...
    public fetchAll(
        ...
        return this.apiService
            ...
            .then(response => {
                const records: Array<ConsultationListingResponse> = response.map((e: any) => {
                    return {
     ...
     * @returns ConsultationListingResponse
     */
    public add(consultationCreateRequest: ConsultationCreateRequest): Promise<boolean> {
        return this.apiService
            .request({
                endpoint: `/consultation`,
                method: `POST`,
    ...
    private mapConsultationResponse(response: any): ConsultationResponse {
        const record: ConsultationResponse = {
            ...
            consultationMethod: response.consultation_method ? response.consultation_method : '',
