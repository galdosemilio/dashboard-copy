...
export class StateFormFieldComponent implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  private updateByCountry(country: string) {
    this.states = country ? STATES.filter(state => state.country === country) : STATES;
  }
}
