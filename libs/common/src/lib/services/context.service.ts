...
import { COOKIE_LANG, COOKIE_ORG, COOKIE_ORG_EXP, CookieService } from './cookie.service';
import { LanguageService } from './language.service';

export type CurrentAccount = AccountSingle & { preferences?: AccountPreferenceSingle };
...
export class ContextService {
  ...
  init() {
    return (): Promise<any> => {
      ...
      if (!cookieOrgId && !baseOrgId) {
        ...
        this.cookie.set(COOKIE_ORG, this.environment.defaultOrgId, COOKIE_ORG_EXP, '/');
        location.href =
          uri.join('/') +
          `?${Object.keys(urlParams)
            .map(key => key + '=' + urlParams[key] + '&')
  ...
  async updateOrganization(org?: SelectedOrganization) {
    ...
    try {
      ...
      if (this.user && org.id && (!org.permissions || org.permissions.viewAll)) {
   ...
   */
  organization$ = new BehaviorSubject<SelectedOrganization>({} as SelectedOrganization);
  ...
  orgHasFoodMode(description: 'Meal' | 'Key-based'): number {
    ...
    return org && org.id && org.preferences
      ? findIndex(org.preferences.food.mode, { description, isActive: true }) + 1
      : 0;
  }
