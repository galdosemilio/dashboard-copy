...
// Main Selector
export const selectUser = createSelector(selectData, (state: ParentState) => state.user);

// utility methods
export const getId = (state: State): string => state.id;

export const getMeasurementPref = (state: State): string => state.measurementPreference;
 ...
 * Inner Selectors
 */
export const selectId: MemoizedSelector<object, string> = createSelector(selectUser, getId);
