/**
 * GET /warehouse/sleep/basic
 */

import { createTest, createValidator, optional } from '@coachcare/backend/tests';
import * as t from 'io-ts';
import { pagination } from '../../../shared/index.test';
import { GetBasicSleepReportsResponse } from './getBasicSleepReports.response';

export const getBasicSleepReportsResponse = createTest<GetBasicSleepReportsResponse>(
  'GetBasicSleepReportsResponse',
  {
    /** Data collection. */
    data: t.array(
      createValidator({
        /** User account data. */
        account: createValidator({
          /** User ID. */
          id: t.string,
          /** First name of the client. */
          firstName: t.string,
          /** Last name of the client. */
          lastName: t.string
        }),
        /** Organization data. */
        organization: createValidator({
          /** Organization ID. */
          id: t.string,
          /** Organization name. */
          name: t.string
        }),
        /** Start date of given unit range. */
        date: t.string,
        /** Sleep data. */
        hoursSlept: createValidator({
          /** Sum of hours slept for given time range. */
          sum: t.number,
          /** Minimum of hours slept for given time range. */
          min: t.number,
          /** Maximum of hours slept for given time range. */
          max: t.number,
          /** Average of hours slept for given time range. */
          avg: t.number,
          /** Number of samples for given time range. */
          sampleCount: t.number
        }),
        /** Assigned provider data. */
        assignedProvider: optional(
          createValidator({
            /** Provider account ID. */
            id: t.string,
            /** First name of the provider. */
            firstName: t.string,
            /** Last name of the provider. */
            lastName: t.string
          })
        )
      })
    ),
    /** Pagination object. */
    pagination: pagination
  }
);
