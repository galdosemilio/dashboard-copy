...
import { OrgEntityExtended, OrgListSegment, PagedResponse } from '@coachcare/backend/services';
...
@Injectable()
export class OrganizationsDataSource extends SearchDataSource<any, any, OrganizationsCriteria> {
  ...
  mapSearch(result: Array<OrgEntityExtended> | Array<GetListSegment>): Array<AutocompleterOption>;
  mapSearch(result) {
    // search handling
    if (this.criteria.isAdmin) {
