...
export class OrganizationsTreeDatabase {
  ...
  getChildClinics(node: ClinicFlatNode) {
    ...
    this.organization
      .getDescendants({ id: parent.org.id, limit: 10, offset: offset })
      .then(res => {
        ...
        if (res.pagination.next) {
          nodes.push(new ClinicNode('LOAD_MORE', 'leafNode', parent.org, node.nodeName));
        }
  ...
  getParentClinic(node: ClinicFlatNode) {
    ...
    if (parent.children.length === 0) {
      this.organization
        .getSingle({ id: node.org.id })
        .then(org => {
          if (org.hierarchyPath.length === 1) {
            parent.childrenChange.next([this.generateNode('NO_PARENT', 'leafNode', parent.org)]);
            this.dataChange.next(this.dataChange.value);
          } else {
            this.organization
              .getSingle({ id: org.hierarchyPath[1] })
              .then(res => {
                parent.childrenChange.next([this.generateNode(res.name, 'leafNode', res)]);
                this.dataChange.next(this.dataChange.value);
              })
              .catch(console.error);
          }
  ...
  private generateNode(nodeName: string, nodeType: string, org: OrganizationSingle): ClinicNode {
