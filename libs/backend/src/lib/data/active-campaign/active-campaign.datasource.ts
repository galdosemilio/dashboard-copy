...
> {
  ...
  mapResult(result: PagedResponse<ActiveCampaignListAssociationItem>): ActiveCampaignListItem[] {
    // pagination handling
    ...
    const mappedData = result.data.map(
      element =>
        new ActiveCampaignListItem(element, { organizationId: this.criteria.organization || '' })
    );
